...
- (void)changeState {
    ...
    // 非空dayView的状态
        ...
        if (_manager.canSelectPastDays == YES && _manager.canSelectAfterDays == NO) {
            // 过去的时间能否点击
            if (_manager.canSelectPastDays &&
                [_manager.helper date:_date isEqualOrBefore:_manager.date] &&
                [_date compare:_manager.date] == NSOrderedAscending) {
                self.enabled = true;
            } else {
                self.enabled = false;
            }
        }else if (_manager.canSelectPastDays == NO && _manager.canSelectAfterDays == YES){
            // 未来的时间能否点击
            if (_manager.canSelectAfterDays && [_manager.helper date:_date isEqualOrAfter:_manager.date] && [_date compare:_manager.date] == NSOrderedDescending) {
                self.enabled = true;
            } else {
                self.enabled = false;
            }
        }else if(_manager.canSelectPastDays == NO && _manager.canSelectAfterDays == NO){
            self.enabled = false;
        }else if (_manager.canSelectPastDays == YES && _manager.canSelectAfterDays == YES){
            self.enabled = true;
        }
