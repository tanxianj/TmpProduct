...
- (void)viewDidLoad {
    ...
    [self SetCALayer];
    [self QrCode];
    [self defaultNavigationBarSetWithTitle:@"优惠核销" Right:@"相册" rightItemBlock:^{
...
//                imagv.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight);
//                [[UIApplication sharedApplication].keyWindow addSubview:imagv];
...
- (void)scanQRCodeWithImage:(UIImage *)image{
    ...
    //监测到的结果数组
    if (![[CIImage imageWithCGImage:image.CGImage] isKindOfClass:UIImage.class]) {
        [XCToast showWithMessage:@"识别失败!"];
        return;
    }
...
//        [MBProgressHUD showInfoMessage:[NSString stringWithFormat:@"扫描结果：%@",scannedResult]];
        if ([scannedResult containsString:@"http"]) {
             ...
             */
            WebLoadingViewController *web = [WebLoadingViewController new];
            web.url = scannedResult;
            [self.navigationController pushViewController:web animated:YES];
            //            [MBProgressHUD showInfoMessage:[NSString stringWithFormat:@"扫描结果：%@",stringValue]];
        }else if([scannedResult containsString:@"uc-"]){
            //            [MBProgressHUD showInfoMessage:[NSString stringWithFormat:@"扫描结果：%@",stringValue]];
            NSString *url = [NSString stringWithFormat:@"/api/coupon/check/%@",[scannedResult substringFromIndex:3]];
            if (!_saoMiaoType) {
                [[HTTPSessionManager sharedManager] POST:url parameters:@{@"shop_id":[UserInfoAndShopModel sharedUserModel].shop.idField} callBack:^(RespondModel *responseModel) {
                    [XCToast showWithMessage:responseModel.message];
                    if (responseModel.status == 200) {
                        KuaiSuSYViewController * vc = [KuaiSuSYViewController new];
                        vc.couponCheckModel = [[CouponCheckModel alloc]initWithDictionary:responseModel.data];
                        vc.type = FastCashRegisterTypeOne;
                ...
                [[HTTPSessionManager sharedManager] POST:url parameters:@{@"shop_id":[UserInfoAndShopModel sharedUserModel].shop.idField} callBack:^(RespondModel *responseModel) {
                    ...
                    if (responseModel.status == 200) {
                        ...
                        //                        vc.couponCheckModel = [[CouponCheckModel alloc]initWithDictionary:responseModel.data];
                        //                        vc.type = FastCashRegisterTypeOne;
                        if (self.backCouponCheckModel) {
                            self.backCouponCheckModel([[CouponCheckModel alloc]initWithDictionary:responseModel.data]);
                        }
...
-(void)SetCALayer{
    ...
    _QrView = [[QRView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
    _QrView.backgroundColor  = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    _QrView.transparentArea = CGSizeMake(kScreenWidth -120, kScreenWidth -120);
    ...
    [_open_or_down mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        NSUInteger bottom =( kScreenHeight -64 )/2 + _QrView.transparentArea.width/2 - 50;
    ...
    [lab mas_makeConstraints:^(MASConstraintMaker *make) {
        NSUInteger bottom =( kScreenHeight -64 )/2 + _QrView.transparentArea.width/2;
        ...
        if (status == AVAuthorizationStatusNotDetermined) {
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
                if (granted) {
                    ...
                    // 用户第一次同意了访问相机权限
                    txjLog(@"用户第一次同意了访问相机权限 - - %@", [NSThread currentThread]);
                    
                } else {
                    // 用户第一次拒绝了访问相机权限
                    txjLog(@"用户第一次拒绝了访问相机权限 - - %@", [NSThread currentThread]);
                }
            }];
...
//            [MBProgressHUD showInfoMessage:[NSString stringWithFormat:@"扫描结果：%@",stringValue]];
        ...
        }else if([stringValue containsString:@"uc-"]){
            txjLog(@"扫描结果：%@",stringValue);     // 可以将字符串放到需要用到的地方（比如label）
//            [MBProgressHUD showInfoMessage:[NSString stringWithFormat:@"扫描结果：%@",stringValue]];
            NSString *url = [NSString stringWithFormat:@"/api/coupon/check/%@",[stringValue substringFromIndex:3]];
            if (!_saoMiaoType) {
                [[HTTPSessionManager sharedManager] POST:url parameters:@{@"shop_id":[UserInfoAndShopModel sharedUserModel].shop.idField} callBack:^(RespondModel *responseModel) {
                    [XCToast showWithMessage:responseModel.message];
                    if (responseModel.status == 200) {
                        ...
                        vc.couponCheckModel = [[CouponCheckModel alloc]initWithDictionary:responseModel.data];
                        vc.type = FastCashRegisterTypeOne;
                        vc.deleteViewCount = 1;
                        [vc defaultNavigationBarSetWithTitle:@"快速收银"];
                        [self.navigationController pushViewController:vc animated:YES];
                    }else{
